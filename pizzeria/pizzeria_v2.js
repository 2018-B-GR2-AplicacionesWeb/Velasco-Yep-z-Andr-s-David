...
function incializarBDD() {
    ...
    return new Promise((resolve, reject) => {
        fs.readFile('bdd.json', 'utf-8', (error, contenidoArchivo) => {
            if (error) {
                fs.writeFile('bdd.json', '{"clientes":[],"pizzas":[]}', (error) => {
                    if (error) {
                        reject({
...
async function main() {
    ...
    respuestaBDD$
        .pipe(preguntarOpcionMenu_Inicio(), opcionesRespuesta_inicio())
        .subscribe(() => { }, (error) => { console.log(error); }, () => { main(); console.log("Completado"); });
}
    ...
    return mergeMap((respuestaBDD) => {
        ...
        switch (opcion) {
            case 'Modulo Pizzas':
                return rxjs
                    .from(incializarBDD())
                    .pipe(preguntarOpciones_crud(), opcionesRespuesta_crud(), ejecutarAccion_crud(), guardarBaseDeDatos());
            case 'Modulo Clientes':
                return rxjs
                    .from(incializarBDD())
                    .pipe(preguntarOpciones_crud(), opcionesRespuesta_crud('cliente'), ejecutarAccion_crud('cliente'), guardarBaseDeDatos());
        }
    });
}
...
function opcionesRespuesta_crud(entidad = 'pizza') {
    return mergeMap((respuestaBDD) => {
...
function actualizar_entidad(respuestaBDD, entidad = 'pizza') {
    return rxjs
...
function consultar_cliente_por_id(respuestaBDD, actualizar = false) {
    return rxjs
        .from(inquirer.prompt(preguntaBuscar_cliente))
        .pipe(mergeMap((respuesta) => {
        //const indice = pizza.id;
        const indiceCliente = respuestaBDD.bdd
            .clientes.findIndex((cliente) => {
...
function ejecutarAccion_crud(entidad = 'pizza') {
